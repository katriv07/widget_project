...
- `get_transactions`: Позволяет распаковывать данные о транзакциях, содержащиеся в JSON-файле.
- `convert_amount_transaction`: Выводит сумму в рублях по курсу на дату совершения транзакции.
- `read_csv_transactions`: Cчитывает финансовые операции из CSV-файла в папке data и выдает список словарей с транзакциям
- `read_excel_transactions`: Cчитывает финансовые операции из Excel-файла в папке data и выдает список словарей с транзакциям
